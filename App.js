  ...
  _renderTextRoom() {
    return (
      <View style={styles.listViewContainer}>
        <ListView
          dataSource={container.ds.cloneWithRows(container.state.textRoomData)}
          renderRow={rowData => <Text>{`${rowData.user}: ${rowData.message}`}</Text>}
          />
        <TextInput
          style={{width: 200, height: 30, borderColor: 'gray', borderWidth: 1}}
          onChangeText={value => container.setState({textRoomValue: value})}
          value={container.state.textRoomValue}
        />
        <TouchableHighlight
          onPress={container._textRoomPress}>
          <Text>Send</Text>
        </TouchableHighlight>
      </View>
    );
...
AppRegistry.registerComponent('webrtctest', () => RCTWebRTCDemo);
